  ...
  Sub <- function(m){
    ...
    sapply(m, function(k) {
      ...
      if (k < n) {
        ...
        # Chat = 1 - (N - k)/N * sum(x/n * exp(lchoose(n - x, k) - lchoose(n - 1, k)))
  ...
  Sub2 <- function(m){
    ...
    sapply(m, function(k) {
      ...
      if (k < nt) {
        ...
        # Chat = 1 - (nT - k)/nT * sum(x/sum(x) * exp(lchoose(nt - x, k) - lchoose(nt - 1, k)))
...
check.datatype <- function(data, datatype, nT = nT, empirical = FALSE) {
  ...
  if (datatype == "incidence_raw") {
    ...
    if (inherits(data, c("data.frame", "matrix"))) {
      data = list('Assemblage_1' = as.matrix(data))
...
", call. = FALSE)
  ...
  if (datatype == "abundance") {
    ...
    if ( ((datatype == "abundance" & sum(sapply(data, sum) < 5) > 0 ) |
          (datatype == "incidence_raw" & sum(sapply(data, sum) < 5) > 0 )) & !empirical  ) stop("To ensure reliable results, iNEXT.3D requires sufficient data; the number of observed species should be at least five.
", call. = FALSE)
